#tag Class
Protected Class Class_UnitMesure
	#tag Method, Flags = &h0
	...
	#tag Method, Flags = &h0
		Function DBase_Create() As Boolean
		  ...
		  rs = App.MainDB.SQLSelect("SELECT COUNT(*) AS DOUBLON FROM UNITMESURE WHERE UPPER(NOMABREG)='"+NomAbreg.Uppercase+"' OR UPPER(NOM)='"+Nom.Uppercase+"'")
		  if  not (rs=NIL) then
		    rs.MoveFirst
		    if rs.Field("DOUBLON").IntegerValue >0 then
		      msgbox("Il existe deja une unité de mesure avec la même abreviation ou le même nom")
		      return false
		    end if
		  end if
		  
		  rs = DBaseID.SQLSelect("SELECT IFNULL(MAX(NUMERO)+1,1) AS MAXNUM FROM UNITMESURE ")
		  ...
		  OrdreSql=" INSERT INTO    UNITMESURE(NUMERO)     VALUES ([NUMERO])    "
		  ...
		  rs=DBaseID.SQLSelect("SELECT * FROM UNITMESURE WHERE NUMERO ="+str(MaxTaxeva))
		  rs.MoveFirst
		  ...
		  if  not (rs=NIL) then
		    ...
		    rs.Field("NOMABREG").Value  = NomAbreg
	...
	#tag Method, Flags = &h21
		Private Sub DBase_MajRecordset()
		  ...
		  DBaseRS.Field("NOMABREG").StringValue  = NomAbreg
	...
	#tag Method, Flags = &h21
		Private Sub MajPropriete()
		  ...
		  NomAbreg  = DBaseRS.Field("NOMABREG").StringValue
		  ...
		  if NomAbreg       = chr(0) then   NomAbreg  = ""
