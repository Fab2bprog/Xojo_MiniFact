#tag Window
Begin Window Win_UnitMesureLst
   BackColor       =   &cFFFFFF00
   Backdrop        =   0
   CloseButton     =   True
   Composite       =   False
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES UNITES DE MESURE"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "UNITES DE MESURE CORRESPONDANTES"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
      ...
      Begin Listbox LST_RESULT
         ...
         InitialValue    =   "Code	Abréviation	Nom	Blocage"
         Italic          =   False
         Left            =   34
         LockBottom      =   True
         LockedInPosition=   False
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CRITERES DE RECHERCHE DES UNITES DE MESURE"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjUnitMesure = new Class_UnitMesure
		  ObjUnitMesure.DBaseID = App.MainDB
		  ObjUnitMesure.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  
		  if not ObjUnitMesure.DBaseReqSrc_Exec then
		    exit sub
		  end if
		  ...
		  For i as Integer = 1 to ObjUnitMesure.DBaseRS.FieldCount
		    if not (left(ObjUnitMesure.DBaseRS.IdxField(i).Name,3)="RSX") then
		      Pop_TypeChamp.AddRow   ObjUnitMesure.DBaseRS.IdxField(i).Name
		    end if
		  Next
	...
	#tag MenuHandler
		Function Ite_ExportXML() As Boolean Handles Ite_ExportXML.Action
			...
			if not ObjExport.Export_XML("UNITMESURE") then
			MsgBox "L'export XML de la page en cour à échoué"
			return false
			end if
	...
	#tag Method, Flags = &h0
		Sub Action_Ajouter()
		  dim test As Boolean
		  dim PermUnitMesureFich as Class_AccesObjet
		  
		  PermUnitMesureFich = new Class_AccesObjet
		  
		  if PermUnitMesureFich.Interoge_Acces("Win_UnitMesureFich","W",false)  then
		    Win_UnitMesureFich.Show
		    Win_UnitMesureFich.AccesPerm_Data=PermUnitMesureFich
		    Win_UnitMesureFich.SourceWindowName=AccessWindow_ID
		    test = Win_UnitMesureFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"ADD", 0)=true
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Action_Blocage(Bloquer as String)
		  ...
		  if  not PermUnitMesureFich.Interoge_Acces("Win_UnitMesureFich","W") or PermUnitMesureFich.Blocage="N"  then
		    RepUser=MsgBox("Vous ne possedez pas les droits necessaire à la réalisation de cette action",48)
		    exit sub
		  end if
		  
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox(LibBloque+" en ecriture le UnitMesure sélectionné ?",52)
		    If  RepUser=7 then
		  ...
		  OrdreSql="UPDATE UNITMESURE SET BLOCAGE ='"+Bloquer+"' WHERE NUMERO="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  LST_RESULT.Cell(LST_RESULT.ListIndex,3)=Bloquer
	...
	#tag Method, Flags = &h0
		Sub Action_Consulter()
		  ...
		  if PermUnitMesureFich.Interoge_Acces("Win_UnitMesureFich","R",False)  then
		    
		    Win_UnitMesureFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_UnitMesureFich.AccesPerm_Data=PermUnitMesureFich
		    Win_UnitMesureFich.SourceWindowName=AccessWindow_ID
		    test = Win_UnitMesureFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"LIRE", numero)=true
	...
	#tag Method, Flags = &h0
		Sub Action_Modifier()
		  ...
		  if PermUnitMesureFich.Interoge_Acces("Win_UnitMesureFich","W",false)  then
		    ...
		    Win_UnitMesureFich.AccesPerm_Data=PermUnitMesureFich
		    Win_UnitMesureFich.SourceWindowName=AccessWindow_ID
		    test = Win_UnitMesureFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"MODIF", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  OrdreSql="DELETE FROM UNITMESURE WHERE NUMERO = "+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjUnitMesure.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjUnitMesure.DBaseSource = Liste_Requete
		  end if
		  ...
		  if not ObjUnitMesure.Charge_Premier then
		    return false
		  end if
		  ...
		  Do
		    LST_RESULT.AddRow  str(ObjUnitMesure.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjUnitMesure.NomAbreg)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjUnitMesure.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,3) = str(ObjUnitMesure.BLOCAGE)
		    if not ObjUnitMesure.Charge_Suivant then
	...
	#tag Method, Flags = &h21
		Private Sub Valide_Ligne()
		  ...
		  if PermUnitMesureFich.Interoge_Acces("Win_UnitMesureFich","W",true)  then
		    Action_Modifier
		  else
		    if PermUnitMesureFich.Interoge_Acces("Win_UnitMesureFich","R",false)  then
		      Action_Consulter
	...
	#tag Property, Flags = &h0
		ObjUnitMesure As Class_UnitMesure
	#tag EndProperty


	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_UnitMesureLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"UNITMESURE", Scope = Private
	#tag EndConstant
