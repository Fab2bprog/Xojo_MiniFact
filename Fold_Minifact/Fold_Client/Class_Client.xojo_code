...
Protected Class Class_Client
	...
	#tag Method, Flags = &h0
		Sub DBase_Create()
		  Dim MaxClient  as int64
		  Dim OrdreSql   as string
		  Dim RepUser    as integer
		  Dim rs         as RecordSet
		  
		  rs = DBaseID.SQLSelect("SELECT IFNULL(MAX(NUMERO)+1,1) AS MAXNUM FROM CLIENT ")
		  
		  if  not (rs=NIL) then
		    rs.MoveFirst
		    MaxClient        = rs.Field("MAXNUM").Value
		  else
		    MaxClient        = 1
		  end if
		  
		  OrdreSql=" INSERT INTO    CLIENT(NUMERO)     VALUES ([NUMERO])    "
		  OrdreSql=ReplaceAll(OrdreSql, "[NUMERO]"          ,str(MaxClient)     )
		  ...
		  rs=DBaseID.SQLSelect("SELECT * FROM CLIENT WHERE NUMERO ="+str(MaxClient))
		  rs.MoveFirst
		  ...
		  Exception err as NilObjectException
		    RepUser = MsgBox ("Echec de la création du client"+EndOfLine+DBaseSource,48,"Erreur execution requete sql")
