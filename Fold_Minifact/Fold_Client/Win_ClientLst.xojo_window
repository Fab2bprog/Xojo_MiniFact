...
Begin Window Win_ClientLst
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES CLIENTS"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CLIENTS CORRESPONDANT"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   " CRITERES DE RECHERCHE CLIENT"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjClient = new Class_Client
		  ObjClient.DBaseID = App.MainDB
		  ObjClient.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  
		  if not ObjClient.DBaseReqSrc_Exec then
		    exit sub
		  end if
		  ...
		  For i as Integer = 1 to ObjClient.DBaseRS.FieldCount
		    if not (left(ObjClient.DBaseRS.IdxField(i).Name,3)="RSX") then
		      Pop_TypeChamp.AddRow   ObjClient.DBaseRS.IdxField(i).Name
		    end if
		  Next
	...
	#tag Method, Flags = &h0
		Sub Action_Ajouter()
		  ...
		  if PermClientFich.Interoge_Acces("Win_ClientFich","W",false)  then
		    Win_ClientFich.Show
		    Win_ClientFich.AccesPerm_Data=PermClientFich
		    test = Win_ClientFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"ADD", 0)=true
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Action_Blocage(Bloquer as String)
		  ...
		  if  not PermClientFich.Interoge_Acces("Win_ClientFich","W") or PermClientFich.Blocage="N"  then
		    RepUser=MsgBox("Vous ne possedez pas les droits necessaire à la réalisation de cette action",48)
		    exit sub
		  end if
		  
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox(LibBloque+" en ecriture le client sélectionné ?",52)
		    If  RepUser=7 then
		  ...
		  OrdreSql="UPDATE CLIENT SET BLOCAGE ='"+Bloquer+"' WHERE NUMERO="+str(numero)
	...
	#tag Method, Flags = &h0
		Sub Action_Consulter()
		  ...
		  if PermClientFich.Interoge_Acces("Win_ClientFich","R",False)  then
		    
		    Win_ClientFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_ClientFich.AccesPerm_Data=PermClientFich
		    test = Win_ClientFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"LIRE", numero)=true
	...
	#tag Method, Flags = &h0
		Sub Action_Modifier()
		  ...
		  if PermClientFich.Interoge_Acces("Win_ClientFich","W",false)  then
		    ...
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_ClientFich.AccesPerm_Data=PermClientFich
		    test = Win_ClientFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"MODIF", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox("Supprimer le client sélectionné ?",52)
		    If  RepUser=7 then
		      exit sub  //User repond non
		    end if
		  else
		  ...
		  OrdreSql="DELETE FROM CLIENT WHERE NUMERO="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  OrdreSql="DELETE FROM CONTACT WHERE NUMCLIENT="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjClient.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjClient.DBaseSource = Liste_Requete
		  end if
		  ...
		  if not ObjClient.Charge_Premier then
		    return false
		  end if
		  ...
		  Do
		    LST_RESULT.AddRow  str(ObjClient.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjClient.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjClient.Adresse1)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,3) = str(ObjClient.Adresse2)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,4) = str(ObjClient.CodePostal)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,5) = str(ObjClient.Ville)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,6) = str(ObjClient.Region)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,7) = str(ObjClient.Pays)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,8) = str(ObjClient.Blocage)
		    if not ObjClient.Charge_Suivant then
		      exit do
		    end if
	...
	#tag Method, Flags = &h21
		Private Sub Valide_Ligne()
		  ...
		  if PermClientFich.Interoge_Acces("Win_ClientFich","W",true)  then
		    Action_Modifier
		  else
		    if PermClientFich.Interoge_Acces("Win_ClientFich","R",false)  then
		      Action_Consulter
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_ClientLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"CLIENT", Scope = Private
	#tag EndConstant
