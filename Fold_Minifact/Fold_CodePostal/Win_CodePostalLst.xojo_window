#tag Window
Begin Window Win_CodePostalLst
   BackColor       =   &cFFFFFF00
   Backdrop        =   0
   CloseButton     =   True
   Composite       =   False
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES CODES POSTAUX ET FRAIS DE LIVRAISON"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CODES POSTAUX CORRESPONDANTS"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
      ...
      Begin Listbox LST_RESULT
         ...
         Bold            =   False
         ColumnCount     =   7
         ColumnWidths    =   "50,100,*,80,100,50,80"
         DataField       =   ""
         DataSource      =   ""
         DefaultRowHeight=   -1
         ...
         InitialValue    =   "Code	CPO	Nom	Departement	Cout_Livraison	Activer	Blocage"
         Italic          =   False
         Left            =   34
         LockBottom      =   True
         LockedInPosition=   False
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CRITERES DE RECHERCHE DES CODES POSTAUX"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjCodePostal = new Class_CodePostal
		  ObjCodePostal.DBaseID = App.MainDB
		  ObjCodePostal.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  
		  if not ObjCodePostal.DBaseReqSrc_Exec then
		    exit sub
		  end if
		  ...
		  For i as Integer = 1 to ObjCodePostal.DBaseRS.FieldCount
		    if not (left(ObjCodePostal.DBaseRS.IdxField(i).Name,3)="RSX") then
		      Pop_TypeChamp.AddRow   ObjCodePostal.DBaseRS.IdxField(i).Name
		    end if
		  Next
	...
	#tag MenuHandler
		Function Ite_ExportXML() As Boolean Handles Ite_ExportXML.Action
			...
			if not ObjExport.Export_XML("CODEPOSTAL") then
			MsgBox "L'export XML de la page en cour à échoué"
			return false
			end if
	...
	#tag Method, Flags = &h0
		Sub Action_Ajouter()
		  dim test As Boolean
		  dim PermCodePostalFich as Class_AccesObjet
		  
		  PermCodePostalFich = new Class_AccesObjet
		  
		  if PermCodePostalFich.Interoge_Acces("Win_CodePostalFich","W",false)  then
		    Win_CodePostalFich.Show
		    Win_CodePostalFich.AccesPerm_Data=PermCodePostalFich
		    Win_CodePostalFich.SourceWindowName=AccessWindow_ID
		    test = Win_CodePostalFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"ADD", 0)=true
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Action_Blocage(Bloquer as String)
		  ...
		  if  not PermCodePostalFich.Interoge_Acces("Win_CodePostalFich","W") or PermCodePostalFich.Blocage="N"  then
		    RepUser=MsgBox("Vous ne possedez pas les droits necessaire à la réalisation de cette action",48)
		    exit sub
		  end if
		  
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox(LibBloque+" en ecriture le CodePostal sélectionné ?",52)
		    If  RepUser=7 then
		  ...
		  OrdreSql="UPDATE CODEPOSTAL SET BLOCAGE ='"+Bloquer+"' WHERE NUMERO="+str(numero)
	...
	#tag Method, Flags = &h0
		Sub Action_Consulter()
		  ...
		  if PermCodePostalFich.Interoge_Acces("Win_CodePostalFich","R",False)  then
		    
		    Win_CodePostalFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_CodePostalFich.AccesPerm_Data=PermCodePostalFich
		    Win_CodePostalFich.SourceWindowName=AccessWindow_ID
		    test = Win_CodePostalFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"LIRE", numero)=true
	...
	#tag Method, Flags = &h0
		Sub Action_Modifier()
		  ...
		  if PermCodePostalFich.Interoge_Acces("Win_CodePostalFich","W",false)  then
		    ...
		    Win_CodePostalFich.AccesPerm_Data=PermCodePostalFich
		    Win_CodePostalFich.SourceWindowName=AccessWindow_ID
		    test = Win_CodePostalFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"MODIF", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox("Supprimer l' Unite de Mesure sélectionné ?"+ EndOfLine+ "Cela entrainera sa suppression de toutes les conditions commerciales d'achat et de vente",52)
		    If  RepUser=7 then
		      exit sub  //User repond non
		    end if
		  else
		  ...
		  OrdreSql="DELETE FROM CODEPOSTAL WHERE NUMERO = "+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  OrdreSql="UPDATE VARACHAT SET UNITACHAT = 0  WHERE UNITACHAT="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  OrdreSql="UPDATE VARVENTE SET UNITVENTE = 0  WHERE UNITVENTE="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjCodePostal.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjCodePostal.DBaseSource = Liste_Requete
		  end if
		  ...
		  if not ObjCodePostal.Charge_Premier then
		    return false
		  end if
		  ...
		  Do
		    ...
		    LST_RESULT.AddRow  str(ObjCodePostal.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjCodePostal.CPO)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjCodePostal.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,3) = str(ObjCodePostal.Departement)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,4) = str(ObjCodePostal.CoutLivr)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,5) = str(ObjCodePostal.Activer)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,6) = str(ObjCodePostal.BLOCAGE)
		    
		    if not ObjCodePostal.Charge_Suivant then
	...
	#tag Method, Flags = &h21
		Private Sub Valide_Ligne()
		  ...
		  if PermCodePostalFich.Interoge_Acces("Win_CodePostalFich","W",true)  then
		    Action_Modifier
		  else
		    if PermCodePostalFich.Interoge_Acces("Win_CodePostalFich","R",false)  then
		      Action_Consulter
	...
	#tag Property, Flags = &h0
		ObjCodePostal As Class_CodePostal
	#tag EndProperty
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_CodePostalLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"CODEPOSTAL", Scope = Private
	#tag EndConstant
