...
Begin Window Win_SecurUserLst
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES UTILISATEURS DU LOGICIEL"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "UTILISATEURS DU LOGICIEL CORRESPONDANTS"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CRITERES DE RECHERCHE DES UTILISATEURS DU LOGICIEL"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjUser.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  ...
		  For i as Integer = 1 to  ObjUser.DBaseRS.FieldCount
		    if not (left(ObjUser.DBaseRS.IdxField(i).Name,3)="RSX") then
		      Pop_TypeChamp.AddRow   ObjUser.DBaseRS.IdxField(i).Name
		    end if
		  Next
	...
	#tag Method, Flags = &h0
		Sub Action_Ajouter()
		  ...
		  if PermFournFich.Interoge_Acces("Win_SecurUserFich","W",False)  then
		    Win_SecurUserFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_SecurUserFich.AccesPerm_Data   = PermFournFich
		    Win_SecurUserFich.SourceWindowName = AccessWindow_ID
		    test = Win_SecurUserFich.AccessQuery_User(App.MyLogUserInfo.Nom,"ADD", 0)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Consulter()
		  ...
		  if PermFournFich.Interoge_Acces("Win_SecurUserFich","R",False)  then
		    Win_SecurUserFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_SecurUserFich.AccesPerm_Data   = PermFournFich
		    Win_SecurUserFich.SourceWindowName = AccessWindow_ID
		    test = Win_SecurUserFich.AccessQuery_User(App.MyLogUserInfo.Nom,"LIRE", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Modifier()
		  ...
		  if PermFournFich.Interoge_Acces("Win_SecurUserFich","W",False)  then
		    ...
		    Win_SecurUserFich.AccesPerm_Data   = PermFournFich
		    Win_SecurUserFich.SourceWindowName = AccessWindow_ID
		    test = Win_SecurUserFich.AccessQuery_User(App.MyLogUserInfo.Nom,"MODIF", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Selection()
		  ...
		  if AccessMode_Data="SELECT" then
		    ...
		    if SourceWindowName ="Win_SecurGroupFich" then
		      
		      if  Win_SecurGroupFich.Utilisateur_Add(  LST_RESULT.Cell(LST_RESULT.ListIndex,0).CDbl ,  LST_RESULT.Cell(LST_RESULT.ListIndex,1) ) =false then
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox("Supprimer l'utilisateur sélectionné ?",52)
		    If  RepUser=7 then
		  ...
		  numero = LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		  if Numero=1 then
		    RepUser=MsgBox("La suppression du membre Administrateur sera toujours interdite par le logiciel."+EndOfLine+_
		    "L'Administrateur est un utilisateur spécial et inamovible protégé du logiciel."+EndOfLine+_
		  ...
		  OrdreSql="DELETE FROM USER WHERE NUMERO="+str(numero)
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjUser.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjUser.DBaseSource = Liste_Requete
		  end if
		  ...
		  Do
		    LST_RESULT.AddRow  str(ObjUser.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjUser.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjUser.Blocage)
		    if not ObjUser.Charge_Suivant then
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_SecurUserLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"USER", Scope = Private
	#tag EndConstant
