...
Begin Window Win_SecurGoupLst
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES GROUPES UTILISATEURS"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "GROUPES UTILISATEURS CORRESPONDANT"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CRITERES DE RECHERCHE DES GROUPES UTILISATEURS"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjGroupe = new Class_GroupUser
		  ObjGroupe.DBaseID = App.MainDB
		  ObjGroupe.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  
		  if not ObjGroupe.DBaseReqSrc_Exec then
		    exit sub
		  end if
		  ...
		  For i as Integer = 1 to  ObjGroupe.DBaseRS.FieldCount
		    if not (left(ObjGroupe.DBaseRS.IdxField(i).Name,3)="RSX") then
		      Pop_TypeChamp.AddRow   ObjGroupe.DBaseRS.IdxField(i).Name
		    end if
		  Next
	...
	#tag Method, Flags = &h0
		Sub Action_Ajouter()
		  ...
		  if PermFournFich.Interoge_Acces("Win_SecurGroupFich","W",False)  then
		    Win_SecurGroupFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_SecurGroupFich.AccesPerm_Data   = PermFournFich
		    Win_SecurGroupFich.SourceWindowName = AccessWindow_ID
		    test = Win_SecurGroupFich.AccessQuery_User(App.MyLogUserInfo.Nom,"ADD", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Consulter()
		  ...
		  if PermFournFich.Interoge_Acces("Win_SecurGroupFich","R",False)  then
		    Win_SecurGroupFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_SecurGroupFich.AccesPerm_Data   = PermFournFich
		    Win_SecurGroupFich.SourceWindowName = AccessWindow_ID
		    test = Win_SecurGroupFich.AccessQuery_User(App.MyLogUserInfo.Nom,"LIRE", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Modifier()
		  ...
		  if PermFournFich.Interoge_Acces("Win_SecurGroupFich","W",False)  then
		    ...
		    Win_SecurGroupFich.AccesPerm_Data   = PermFournFich
		    Win_SecurGroupFich.SourceWindowName = AccessWindow_ID
		    test = Win_SecurGroupFich.AccessQuery_User(App.MyLogUserInfo.Nom,"MODIF", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Selection()
		  ...
		  if AccessMode_Data="SELECT" then
		    ...
		    if SourceWindowName ="Win_SecurObjetFich" then
		      
		      if Win_SecurObjetFich.Groupe_Add(  LST_RESULT.Cell(LST_RESULT.ListIndex,0).CDbl ,  LST_RESULT.Cell(LST_RESULT.ListIndex,1) ) =false then
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox("Supprimer le groupe sélectionné ?",52)
		    If  RepUser=7 then
		  ...
		  numero = LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		  if numero=1 then
		    RepUser=MsgBox("La suppression du groupe Administrateurs sera toujours interdite par le logiciel."+EndOfLine+_
		    "Le groupe Administrateurs est un groupe spécial protégé du logiciel."+EndOfLine+_
		    "Son existence est obligatoire et vous est imposée pour le bon fonctionnement du logiciel.",16,"Operation Interdite")
		    exit sub
		  end if
		  ...
		  OrdreSql="DELETE FROM USERGROUP WHERE  NUMERO   = "+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  OrdreSql="DELETE FROM OBJETGROUP WHERE NUMGROUP = "+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjGroupe.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjGroupe.DBaseSource = Liste_Requete
		  end if
		  ...
		  if not ObjGroupe.Charge_Premier then
		    return false
		  end if
		  ...
		  Do
		    LST_RESULT.AddRow  str(ObjGroupe.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjGroupe.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjGroupe.Blocage)
		    if not ObjGroupe.Charge_Suivant then
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_SecurGroupLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"USERGROUP", Scope = Private
	#tag EndConstant
