...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function AccessQuery_User(Login_user as String, DataAccessMode as String, numCaract as Int64) As Boolean
		  dim test as Boolean
		  ...
		  if AccessMode_Data="ADD" then
		    ...
		    CaractActuel=new Class_Caract
		    CaractActuel.DBaseID=App.MainDB
		    return true
		  end if
		  ...
		  CaractActuel = new Class_Caract
		  CaractActuel.DBaseID=App.MainDB
		  CaractActuel.DBaseSource="SELECT * FROM CARACT WHERE NUMERO="+str(numCaract)
		  
		  if not CaractActuel.DBaseReqSrc_Exec then
		    return false
		  end if
		  
		  test=CaractActuel.Charge_Premier
	...
	#tag Method, Flags = &h0
		Sub EcranVersObjet()
		  CaractActuel.Numero      = Cdbl(Txt_Code.Text)
		  CaractActuel.Nom         = Txt_Nom.Text
	...
	#tag Method, Flags = &h0
		Sub ObjetVersEcran()
		  Txt_Code.Text              =    str(CaractActuel.Numero)
		  Txt_Nom.Text               =    CaractActuel.Nom
		  
		  if CaractActuel.BLOCAGE="O" then
	...
	#tag Method, Flags = &h0
		Sub Synchro_VarRes()
		  ...
		  VarRes_Code           =    str(CaractActuel.Numero)
		  VarRes_Nom            =    CaractActuel.Nom
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_CaractFich", Scope = Private
	#tag EndConstant
	...
	#tag Event
		Sub Action()
		  ...
		  if Valide_Formulaire then
		    ...
		    If AccessMode_Data="MODIF" then CaractActuel.DBase_Update
		    If AccessMode_Data="ADD"   then
		      if CaractActuel.DBase_Create = false then
		        exit sub
		      end if
		    end if
		    
		    if SourceWindowName="Win_CaractLst" then
		      BolVoid = Win_CaractLst.Page_Lire
		    end if
		...
		Sub Action()
		  if me.Icon=lock_32X then
		    ...
		    if AccesPerm_Data.Blocage="O" And AccesPerm_Data.ModeAcces="W" then
		      CaractActuel.BLOCAGE="N"
		      Enable_Formulaire(True)
		    end if
		  else
		    me.Icon=lock_32X
		    If AccesPerm_Data.Blocage="O" And AccesPerm_Data.ModeAcces="W" then
		      CaractActuel.BLOCAGE="O"
		      Enable_Formulaire(False)
		    end if
		  end if
