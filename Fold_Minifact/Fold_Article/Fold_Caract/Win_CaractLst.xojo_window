...
Begin Window Win_CaractLst
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES CARACTERISTIQUES POSSIBLES DES ARTICLES"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CARACTERISTIQUES CORRESPONDANTES"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CRITERES DE RECHERCHE CARACTERISTIQUE"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjCaract.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  ...
		  For i as Integer = 1 to ObjCaract.DBaseRS.FieldCount
		    if not (left(ObjCaract.DBaseRS.IdxField(i).Name,3)="RSX") then
		      Pop_TypeChamp.AddRow   ObjCaract.DBaseRS.IdxField(i).Name
		    end if
		  Next
	...
	#tag MenuHandler
		Function Ite_ExportXML() As Boolean Handles Ite_ExportXML.Action
			...
			if not ObjExport.Export_XML("Caract") then
			MsgBox "L'export XML de la page en cour à échoué"
			return false
			end if
	...
	#tag Method, Flags = &h0
		Sub Action_Ajouter()
		  dim test As Boolean
		  dim PermCaractFich as Class_AccesObjet
		  
		  PermCaractFich = new Class_AccesObjet
		  
		  if PermCaractFich.Interoge_Acces("Win_CaractFich","W",false)  then
		    Win_CaractFich.Show
		    Win_CaractFich.AccesPerm_Data=PermCaractFich
		    Win_CaractFich.SourceWindowName=AccessWindow_ID
		    test = Win_CaractFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"ADD", 0)=true
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Action_Blocage(Bloquer as String)
		  ...
		  if  not PermCaractFich.Interoge_Acces("Win_CaractFich","W") or PermCaractFich.Blocage="N"  then
		    RepUser=MsgBox("Vous ne possedez pas les droits necessaire à la réalisation de cette action",48)
		    exit sub
		  end if
		  
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox(LibBloque+" en ecriture le Caract sélectionné ?",52)
		    If  RepUser=7 then
		  ...
		  OrdreSql="UPDATE CARACT SET BLOCAGE ='"+Bloquer+"' WHERE NUMERO="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  LST_RESULT.Cell(LST_RESULT.ListIndex,2)=Bloquer
	...
	#tag Method, Flags = &h0
		Sub Action_Consulter()
		  ...
		  if PermCaractFich.Interoge_Acces("Win_CaractFich","R",False)  then
		    
		    Win_CaractFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_CaractFich.AccesPerm_Data=PermCaractFich
		    Win_CaractFich.SourceWindowName=AccessWindow_ID
		    test = Win_CaractFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"LIRE", numero)=true
	...
	#tag Method, Flags = &h0
		Sub Action_Modifier()
		  ...
		  if PermCaractFich.Interoge_Acces("Win_CaractFich","W",false)  then
		    ...
		    Win_CaractFich.AccesPerm_Data=PermCaractFich
		    Win_CaractFich.SourceWindowName=AccessWindow_ID
		    test = Win_CaractFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"MODIF", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox("Supprimer le Caract sélectionné ?",52)
		    If  RepUser=7 then
		      exit sub  //User repond non
		    end if
		  else
		  ...
		  OrdreSql="DELETE FROM CARACT WHERE NUMERO="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  OrdreSql="DELETE FROM ARTCARACT WHERE NUMCARACT="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjCaract.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjCaract.DBaseSource = Liste_Requete
		  end if
		  ...
		  Do
		    LST_RESULT.AddRow  str(ObjCaract.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjCaract.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjCaract.BLOCAGE)
		    if not ObjCaract.Charge_Suivant then
	...
	#tag Method, Flags = &h21
		Private Sub Valide_Ligne()
		  ...
		  if PermCaractFich.Interoge_Acces("Win_CaractFich","W",true)  then
		    Action_Modifier
		  else
		    if PermCaractFich.Interoge_Acces("Win_CaractFich","R",false)  then
		      Action_Consulter
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_CaractLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"CARACT", Scope = Private
	#tag EndConstant
