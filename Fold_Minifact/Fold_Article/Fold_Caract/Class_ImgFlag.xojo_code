...
Protected Class Class_ImgFlag
	...
	#tag Method, Flags = &h0
		Sub DBase_Create()
		  ...
		  rs = DBaseID.SQLSelect("SELECT IFNULL(MAX(NUMERO)+1,1) AS MAXNUM FROM IMGFLAG ")
		  ...
		  OrdreSql=" INSERT INTO    IMGFLAG(NUMERO)     VALUES ([NUMERO])    "
		  ...
		  rs=DBaseID.SQLSelect("SELECT * FROM IMGFLAG WHERE NUMERO ="+str(MaxTaxeva))
		  rs.MoveFirst
		  ...
		  Exception err as NilObjectException
		    RepUser = MsgBox ("Echec de la création de la vue de l'image "+EndOfLine+DBaseSource,48,"Erreur execution requete sql")
