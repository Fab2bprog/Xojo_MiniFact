#tag Class
Protected Class Class_ArtCaract
	#tag Method, Flags = &h0
	...
	#tag Method, Flags = &h0
		Sub DBase_Create()
		  ...
		  OrdreSql=" INSERT OR IGNORE INTO "+Table+"(NUMART,NUMCARACT) VALUES ([NUMART],[NUMCARACT]) "
		  OrdreSql= ReplaceAll(OrdreSql, "[NUMART]"      , str(NumArt)     )
		  OrdreSql= ReplaceAll(OrdreSql, "[NUMCARACT]"   , str(NumCaract)  )
		  ...
		  rs=DBaseID.SQLSelect("SELECT * FROM "+Table+" WHERE NUMART ="+str(NumArt)+" AND NUMCARACT ="+str(NumCaract)  )
		  rs.MoveFirst
		  
		  if  not (rs=NIL) then
		    
		    rs.Edit
		    rs.Field("VALEUR").StringValue               = Valeur
		    rs.Update
		  ...
		  Exception err as NilObjectException
		    RepUser = MsgBox ("Echec de la prise en compte de la caracteristique "+EndOfLine+DBaseSource,48,"Erreur execution requete sql")
	...
	#tag Method, Flags = &h0
		Sub DBase_Delete(Numero as Int64 = 0)
		  ...
		  OrdreSql = " DELETE FROM "+Table+" WHERE NUMCARACT  =  " + str(Numero)
	...
	#tag Method, Flags = &h21
		Private Sub DBase_MajRecordset()
		  ...
		  DBaseRS.Field("NUMCARACT").Int64Value  = NumCaract
		  DBaseRS.Field("VALEUR").StringValue    = Valeur
	...
	#tag Method, Flags = &h0
		Sub DBase_Update(RefArt as Int64 = - 1, RefCaract as Int64 = - 1)
		  Dim OrdreSql as String
		  
		  if RefArt = -1 OR RefCaract=-1 then
		    DBaseRS.Edit
		    DBase_MajRecordset
		    DBaseRS.Update
		    DBaseID.Commit
		  ...
		  OrdreSql = " UPDATE " + Table +  " SET NUMART     =  " + str(NumArt)    +"  ,"+ _
		  "                                   NUMCARACT  =  " + str(NumCaract) +"  ,"+ _
		  "                                   VALEUR     = '" + Valeur         +"'  "+ _
		  " WHERE NUMART =  " + str(RefArt)   +" AND  NUMCARACT ="+str(RefCaract)
	...
	#tag Method, Flags = &h21
		Private Sub MajPropriete()
		  ...
		  NumCaract = DBaseRS.Field("NUMCARACT").Int64Value
		  Valeur    = DBaseRS.Field("VALEUR").StringValue
		  Blocage   = DBaseRS.Field("BLOCAGE").StringValue
		  Nom       = NomCaract(NumCaract)
		  
		  // Gestion hypothetique des valeurs null
		  
		  if str(NumArt)    = chr(0) then   NumArt    = 0
		  if str(NumCaract) = chr(0) then   NumCaract = 0
		  if Valeur         = chr(0) then   Valeur    = ""
