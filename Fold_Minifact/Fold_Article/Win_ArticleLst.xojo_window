...
Begin Window Win_ArticleLst
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES ARTICLES"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "ARTICLES CORRESPONDANT"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CRITERES DE RECHERCHE ARTICLE"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjArticle = new Class_Article
		  ObjArticle.DBaseID = App.MainDB
		  ObjArticle.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  
		  if not ObjArticle.DBaseReqSrc_Exec then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox("Supprimer l'article sélectionné ?"+ EndOfLine+ "Cela entrainera la suppression de cet article des stocks",52)
		    If  RepUser=7 then
		      exit sub  //User repond non
		    end if
		  else
		  ...
		  OrdreSql="DELETE FROM STOCK WHERE NUMART="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Sub Def_TypeChamp()
		  ...
		  Pop_TypeChamp.AddRow      "Numero"
		  Pop_TypeChamp.RowTag(0) = "NUMERO"
		  ...
		  Pop_TypeChamp.AddRow      "Description"
		  Pop_TypeChamp.RowTag(3) = "DESCRIPTION"
		  
		  Pop_TypeChamp.AddRow      "Blocage"
		  Pop_TypeChamp.RowTag(4) = "BLOCAGE"
	...
	#tag Method, Flags = &h0
		Sub Lance_Recherche(AffichTout as Boolean = false)
		  ...
		  if not AffichTout then
		    ...
		    Liste_Requete="select *  from "+TableCherche+" WHERE "+  AppliOperation2ChampSql(Pop_TypeChamp.RowTag(Pop_TypeChamp.ListIndex), Pop_Operateur.Text ,RechTxt)+" ORDER BY NUMERO  "
		  else
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjArticle.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjArticle.DBaseSource = Liste_Requete
		  end if
		  ...
		  if not ObjArticle.Charge_Premier then
		    return false
		  end if
		  ...
		  Do
		    LST_RESULT.AddRow  str(ObjArticle.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjArticle.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjArticle.Blocage)
		    if not ObjArticle.Charge_Suivant then
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_ArticleLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"ARTICLE", Scope = Private
	#tag EndConstant
