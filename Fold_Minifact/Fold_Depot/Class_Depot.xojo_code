...
Protected Class Class_Depot
	...
	#tag Method, Flags = &h0
		Function DBase_Create() As Boolean
		  ...
		  rs = App.MainDB.SQLSelect("SELECT COUNT(*) AS DOUBLON FROM DEPOT WHERE UPPER(NOM)='"+Nom.Uppercase+"'")
		  if  not (rs=NIL) then
		    rs.MoveFirst
		    if rs.Field("DOUBLON").IntegerValue >0 then
		      msgbox("Il existe deja un depot qui porte le même nom. ")
		      return false
		    end if
		  end if
		  
		  rs = DBaseID.SQLSelect("SELECT IFNULL(MAX(NUMERO)+1,1) AS MAXNUM FROM DEPOT ")
		  ...
		  OrdreSql=" INSERT INTO    DEPOT(NUMERO)     VALUES ([NUMERO])    "
		  ...
		  rs=DBaseID.SQLSelect("SELECT * FROM DEPOT WHERE NUMERO ="+str(MaxTaxeva))
		  rs.MoveFirst
