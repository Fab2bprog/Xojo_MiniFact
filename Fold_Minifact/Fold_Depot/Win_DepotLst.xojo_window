...
Begin Window Win_DepotLst
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   " RECHERCHE DES DEPOTS"
   Visible         =   True
   Width           =   800
   Begin GroupBox Grb_Resultat
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "DEPOTS CORRESPONDANTS"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
   ...
   Begin GroupBox Grb_Recherche
      AllowAutoDeactivate=   True
      Bold            =   False
      Caption         =   "CRITERES DE RECHERCHE DES DEPOTS"
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
      FontUnit        =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  ObjDepot.DBaseSource="select * from "+TableCherche+" LIMIT 1"
		  ...
		  For i as Integer = 1 to ObjDepot.DBaseRS.FieldCount
		    if not (left(ObjDepot.DBaseRS.IdxField(i).Name,3)="RSX") then
		      Pop_TypeChamp.AddRow   ObjDepot.DBaseRS.IdxField(i).Name
		    end if
		  Next
	...
	#tag MenuHandler
		Function Ite_ExportXML() As Boolean Handles Ite_ExportXML.Action
			...
			if not ObjExport.Export_XML("DEPOT") then
			MsgBox "L'export XML de la page en cour à échoué"
			return false
			end if
	...
	#tag Method, Flags = &h0
		Sub Action_Ajouter()
		  dim test As Boolean
		  dim PermDepotFich as Class_AccesObjet
		  
		  PermDepotFich = new Class_AccesObjet
		  
		  if PermDepotFich.Interoge_Acces("Win_DepotFich","W",false)  then
		    Win_DepotFich.Show
		    Win_DepotFich.AccesPerm_Data=PermDepotFich
		    Win_DepotFich.SourceWindowName=AccessWindow_ID
		    test = Win_DepotFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"ADD", 0)=true
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Action_Blocage(Bloquer as String)
		  ...
		  if  not PermDepotFich.Interoge_Acces("Win_DepotFich","W") or PermDepotFich.Blocage="N"  then
		    RepUser=MsgBox("Vous ne possedez pas les droits necessaire à la réalisation de cette action",48)
		    exit sub
		  end if
		  
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox(LibBloque+" en ecriture le Depot sélectionné ?",52)
		    If  RepUser=7 then
		  ...
		  OrdreSql="UPDATE DEPOT SET BLOCAGE ='"+Bloquer+"' WHERE NUMERO="+str(numero)
	...
	#tag Method, Flags = &h0
		Sub Action_Consulter()
		  ...
		  if PermDepotFich.Interoge_Acces("Win_DepotFich","R",False)  then
		    
		    Win_DepotFich.Show
		    numero= LST_RESULT.Cell(LST_RESULT.ListIndex,0).CLong
		    Win_DepotFich.AccesPerm_Data=PermDepotFich
		    Win_DepotFich.SourceWindowName=AccessWindow_ID
		    test = Win_DepotFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"LIRE", numero)=true
	...
	#tag Method, Flags = &h0
		Sub Action_Modifier()
		  ...
		  if PermDepotFich.Interoge_Acces("Win_DepotFich","W",false)  then
		    ...
		    Win_DepotFich.AccesPerm_Data=PermDepotFich
		    Win_DepotFich.SourceWindowName=AccessWindow_ID
		    test = Win_DepotFich.AccessQuery_User(str(App.MyLogUserInfo.Numero),"MODIF", numero)=true
		  end if
	...
	#tag Method, Flags = &h0
		Sub Action_Supprimer()
		  ...
		  if LST_RESULT.SelCount >0 Then
		    RepUser=MsgBox("Supprimer le dépot sélectionné ?"+ EndOfLine+ "Cela entrainera la suppression des stocks d'articles contenus dans ce dépot",52)
		    If  RepUser=7 then
		      exit sub  //User repond non
		    end if
		  else
		  ...
		  OrdreSql="DELETE FROM DEPOT WHERE NUMERO  ="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
		  
		  OrdreSql="DELETE FROM STOCK  WHERE NUMDEPOT ="+str(numero)
		  if not app.RequeteSql(OrdreSql) then
		    exit sub
		  end if
	...
	#tag Method, Flags = &h0
		Function Page_Lire() As Boolean
		  ...
		  if Ligne_ParPage >0 then
		    ObjDepot.DBaseSource    = Liste_Requete+" LIMIT "+str(Ligne_DebPage)+","+str(Ligne_FinPage)
		  else
		    ObjDepot.DBaseSource = Liste_Requete
		  end if
		  ...
		  Do
		    LST_RESULT.AddRow  str(ObjDepot.Numero)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,1) = str(ObjDepot.Nom)
		    LST_RESULT.Cell(LST_RESULT.LastIndex,2) = str(ObjDepot.BLOCAGE)
		    if not ObjDepot.Charge_Suivant then
	...
	#tag Method, Flags = &h21
		Private Sub Valide_Ligne()
		  ...
		  if PermDepotFich.Interoge_Acces("Win_DepotFich","W",true)  then
		    Action_Modifier
		  else
		    if PermDepotFich.Interoge_Acces("Win_DepotFich","R",false)  then
		      Action_Consulter
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_DepotLst", Scope = Private
	#tag EndConstant

	#tag Constant, Name = TableCherche, Type = String, Dynamic = False, Default = \"DEPOT", Scope = Private
	#tag EndConstant
