...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function AccessQuery_User(Login_user as String, DataAccessMode as String, numDepot as Int64) As Boolean
		  dim test as Boolean
		  ...
		  if AccessMode_Data="ADD" then
		    ...
		    DepotActuel=new Class_Depot
		    DepotActuel.DBaseID=App.MainDB
		  ...
		  DepotActuel = new Class_Depot
		  DepotActuel.DBaseID=App.MainDB
		  DepotActuel.DBaseSource="SELECT * FROM DEPOT WHERE NUMERO="+str(numDepot)
		  
		  if not DepotActuel.DBaseReqSrc_Exec then
		    return false
		  end if
		  
		  test=DepotActuel.Charge_Premier
	...
	#tag Method, Flags = &h0
		Sub EcranVersObjet()
		  DepotActuel.Numero      = Cdbl(Txt_Code.Text)
		  DepotActuel.Nom         = Txt_Nom.Text
	...
	#tag Method, Flags = &h0
		Sub ObjetVersEcran()
		  Txt_Code.Text              =    str(DepotActuel.Numero)
		  Txt_Nom.Text               =    DepotActuel.Nom
		  
		  if DepotActuel.BLOCAGE="O" then
	...
	#tag Method, Flags = &h0
		Sub Synchro_VarRes()
		  ...
		  VarRes_Code           =    str(DepotActuel.Numero)
		  VarRes_Nom            =    DepotActuel.Nom
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_DepotFich", Scope = Private
	#tag EndConstant
	...
	#tag Event
		Sub Action()
		  ...
		  if Valide_Formulaire then
		    ...
		    If AccessMode_Data="MODIF" then DepotActuel.DBase_Update
		    If AccessMode_Data="ADD"   then
		      if DepotActuel.DBase_Create=false then
		        exit sub
		      end if
		    end if
		    
		    if SourceWindowName="Win_DepotLst" then
		      BolVoid = Win_DepotLst.Page_Lire
		    end if
		...
		Sub Action()
		  if me.Icon=lock_32X then
		    ...
		    if AccesPerm_Data.Blocage="O" And AccesPerm_Data.ModeAcces="W" then
		      DepotActuel.BLOCAGE="N"
		      Enable_Formulaire(True)
		    end if
		  else
		    me.Icon=lock_32X
		    If AccesPerm_Data.Blocage="O" And AccesPerm_Data.ModeAcces="W" then
		      DepotActuel.BLOCAGE="O"
		      Enable_Formulaire(False)
		    end if
		  end if
