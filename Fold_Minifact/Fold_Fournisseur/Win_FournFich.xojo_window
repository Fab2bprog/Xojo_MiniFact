...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function AccessQuery_User(Login_user as String, DataAccessMode as String, numFourn as Int64) As Boolean
		  dim test as Boolean
		  ...
		  if AccessMode_Data="ADD" then
		    ...
		    FournActuel=new Class_Fourn
		    FournActuel.DBaseID=App.MainDB
		    
		    Contact_Tab = new Class_TabContactFourn
		  ...
		  FournActuel = new Class_Fourn
		  FournActuel.DBaseID=App.MainDB
		  FournActuel.DBaseSource="SELECT * FROM FOURNISSEUR WHERE NUMERO="+str(numFourn)
		  
		  if not FournActuel.DBaseReqSrc_Exec then
		    return false
		  end if
		  
		  test=FournActuel.Charge_Premier
		  
		  Contact_Tab = new Class_TabContactFourn
		  Contact_Tab.Fourn=FournActuel.Numero
	...
	#tag Method, Flags = &h0
		Sub EcranVersObjet()
		  FournActuel.Numero           = Txt_Code.Text.CDbl
		  FournActuel.Nom              = Txt_Nom.Text
		  FournActuel.CodePostal       = Txt_CodePostal.Text
		  FournActuel.Pays             = Txt_Pays.Text
		  FournActuel.Region           = Txt_Region.Text
		  FournActuel.RemiseGlobale    = Txt_Remise.Text.CDbl
		  FournActuel.Email            = Txt_EMail.Text
		  FournActuel.Ville            = Txt_Ville.Text
		  FournActuel.TVAIntraCom      = Txt_NumTva.Text
		  FournActuel.SiteInternet     = Txt_SiteInternet.Text
		  FournActuel.Adresse1         = Txt_Adresse1.Text
		  FournActuel.Adresse2         = Txt_Adresse2.Text
		  
		  FournActuel.Commentaires    = TAr_Commentaires.StyledText.RTFData
	...
	#tag Method, Flags = &h0
		Sub ObjetVersEcran()
		  Txt_Code.Text         = str(FournActuel.Numero)
		  Txt_Nom.Text          = FournActuel.Nom
		  Txt_CodePostal.Text   = FournActuel.CodePostal
		  Txt_Pays.Text         = FournActuel.Pays
		  Txt_Region.Text       = FournActuel.Region
		  Txt_Remise.Text       = str( FournActuel.RemiseGlobale)
		  Txt_EMail.Text        = FournActuel.Email
		  Txt_Ville.Text        = FournActuel.Ville
		  Txt_NumTva.Text       = FournActuel.TVAIntraCom
		  Txt_SiteInternet.Text = FournActuel.SiteInternet
		  Txt_Adresse1.Text     = FournActuel.Adresse1
		  Txt_Adresse2.Text     = FournActuel.Adresse2
		  
		  TAr_Commentaires.StyledText.RTFData = FournActuel.Commentaires
		  
		  if FournActuel.BLOCAGE="O" then
	...
	#tag Method, Flags = &h0
		Sub Synchro_VarRes()
		  ...
		  VarRes_Code         = str(FournActuel.Numero)
		  VarRes_Nom          = FournActuel.Nom
		  VarRes_CodePostal   = FournActuel.CodePostal
		  VarRes_Pays         = FournActuel.Pays
		  VarRes_Region       = FournActuel.Region
		  VarRes_Remise       = str( FournActuel.RemiseGlobale)
		  VarRes_EMail        = FournActuel.Email
		  VarRes_Ville        = FournActuel.Ville
		  VarRes_NumTva       = FournActuel.TVAIntraCom
		  VarRes_SiteInternet = FournActuel.SiteInternet
		  VarRes_Adresse1     = FournActuel.Adresse1
		  VarRes_Adresse2     = FournActuel.Adresse2
		  
		  VarRes_Commentaires =  FournActuel.Commentaires
		End Sub
	#tag EndMethod
	...
	#tag Property, Flags = &h0
		ContactActuel As Class_ContactFourn
	#tag EndProperty

	#tag Property, Flags = &h0
		Contact_Tab As Class_TabContactFourn
	#tag EndProperty
	...
	#tag Constant, Name = AccessWindow_ID, Type = String, Dynamic = False, Default = \"Win_FournFich", Scope = Private
	#tag EndConstant
...
#tag Events Txt_CodePostal
	...
	#tag Event
		Sub LostFocus()
		  if App.Valide_TxtNumLostFocus(me,false,5,0,99999,0) then
		  end if
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events Txt_Ville
	#tag Event
		Function KeyDown(Key As String) As Boolean
		  if Touche_Valide(Key) then
		    FocusNext
...
#tag Events Txt_ContactNom
	...
	#tag EndEvent
#tag EndEvents
#tag Events Txt_ContactFonction
	#tag Event
		Function KeyDown(Key As String) As Boolean
		  if Touche_Valide(Key) then
		    FocusNext
...
#tag Events Txt_ContactEMail
	...
	#tag EndEvent
#tag EndEvents
#tag Events Txt_ContactTelBureau
	#tag Event
		Function KeyDown(Key As String) As Boolean
		  if Touche_Valide(Key) then
		    FocusNext
	...
	#tag EndEvent
#tag EndEvents
#tag Events Txt_ContactTelPortable
	#tag Event
		Function KeyDown(Key As String) As Boolean
		  if Touche_Valide(Key) then
		    FocusNext
...
#tag Events LBTN_DelContact
	#tag Event
		Sub Action()
		  ...
		  if Lst_Contact.SelCount >0 then
		    RepUser=MsgBox("Supprimer ce contact de la liste ?"+EndOfLine+"Ces modification ne seront enregistré qu'à la validation de la fiche fournisseur",52)
...
#tag Events LBTN_ContactCancel
	...
	#tag EndEvent
#tag EndEvents
#tag Events BTN_EMAIL1
	#tag Event
		Sub Action()
		  ShowURL "mailto://"+Txt_ContactEMail.Text
	...
	#tag Event
		Sub Action()
		  ...
		  if Valide_Formulaire then
		    ...
		    If AccessMode_Data="MODIF" then FournActuel.DBase_Update
		    If AccessMode_Data="ADD"   then
		      FournActuel.DBase_Create
		      Contact_Tab.Tab_AffecteFournTemp(FournActuel.Numero)
		    end if
		    
		    
		    if SourceWindowName="Win_FournLst" then
		      BolVoid = Win_FournLst.Page_Lire
		    end if
		...
		Sub Action()
		  if me.Icon=lock_32X then
		    ...
		    if AccesPerm_Data.Blocage="O" And AccesPerm_Data.ModeAcces="W" then
		      FournActuel.BLOCAGE="N"
		      Enable_Formulaire(True)
		    end if
		  else
		    me.Icon=lock_32X
		    If AccesPerm_Data.Blocage="O" And AccesPerm_Data.ModeAcces="W" then
		      FournActuel.BLOCAGE="O"
		      Enable_Formulaire(False)
		    end if
		  end if
